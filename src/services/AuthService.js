...
class AuthService {
    ...
    async login(body) {
        ...
        try {
            ...
            if (token_fcm) {
                let checkFCM = account.fcmtokens.filter(
                    (i) => i == token_fcm,
                )[0];
                //console.log("===> checkFCM", checkFCM);
                if (!checkFCM && token_fcm) {
                    account.fcmtokens = [...account.fcmtokens, token_fcm];
                    account.fcmtokens = account.fcmtokens.slice(
                        Math.max(account.fcmtokens.lenght - 3, 0),
                    );
                    await this.userService.update(account._id, {
            ...
            console.log('===> cacheUser', cacheUser.notification);
    ...
    async logout(token, fcmtoken, account) {
        try {
            ...
            account.fcmtokens = account.fcmtokens?.filter(
                (item) => item != fcmtoken,
            );
            await this.userService.update(account._id, {
                fcmtokens: account.fcmtokens,
